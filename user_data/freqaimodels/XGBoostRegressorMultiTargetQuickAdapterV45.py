...
class XGBoostRegressorMultiTargetQuickAdapterV45(BaseRegressionModel):
    ...
    def objective(self, trial, unfiltered_dataframe, pair, dk, timerange):
        ...
        # get the optuna configuration section
        optuna_config = self.freqai_info.get('optuna_config', {})

        # get the search parameters for train period candles
        ...
        # get the search parameters for train target kernel
        ...
        window = trial.suggest_int('train_period_candles', train_period_candles_min, train_period_candles_max, step=train_period_candles_step)
        kernel = trial.suggest_int('kernel', train_target_kernel_min, train_target_kernel_max, step=train_target_kernel_step)
